  ...
  try {
    ...
    const query = `
      INSERT INTO focosDiarios (
        estado, bioma, regiao, dia, mes, quantidade_focos, ano
      )
      VALUES (?, ?, ?, ?, ?, ?, ?)`
...
const main = async () => {
  try {
    ...
    if (filePath) {
      ...
      console.log('Processamento do dia concluído com sucesso.')
    } else {
      console.error('Não foi possível baixar os dados do dia.')
    }
  } catch (error) {
